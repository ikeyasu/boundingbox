(function($) {
    ...
    BBoxSelector.prototype.create_label_box = function(options) {
      ...
      switch (options.input_method) {
        case 'select':
          ...
          this.label_input = $('<select class="label_input browser-default" name="label"><\/select>');
        ...
        case 'text':
          ...
          this.label_input = $('<input class="teal lighten-5" name="label" ' + 'type="text" value>');
          this.label_box.append(this.label_input);
          break;
        case 'fixed':
  ...
  this.BBoxAnnotator = (function() {
    ...
    BBoxAnnotator.prototype.initialize_events = function(selector, options) {
      ...
      annotator = this;
      this.annotator_element.bind({
        "mousedown touchstart": function(e) {
          if ("changedTouches" in e.originalEvent && e.originalEvent.changedTouches.length > 1) {
            return true;
	  }
          if (!annotator.hit_menuitem) {
            switch (status) {
              ...
              case 'input':
                ...
                var touch = "pageX" in e ? e : e.originalEvent.changedTouches[0]
                selector.start(touch.pageX, touch.pageY);
                status = 'hold';
            }
          }
        ...
        "touchmove": function(e) {
          ...
          switch (status) {
            case 'hold':
              ...
              selector.update_rectangle(Math.round(touch.pageX), Math.round(touch.pageY));
	      return false;
          }
          return true;
        }
      ...
      $(window).bind({
        ...
        "mouseup touchend": function(e) {
          ...
          switch (status) {
            case 'hold':
              var touch = "pageX" in e ? e : e.originalEvent.changedTouches[0]
              selector.update_rectangle(touch.pageX, touch.pageY);
      ...
      selector.get_input_element().bind({"mousedown mousemove mouseup touchstart touchmove touchend": function(e) {
        return annotator.hit_menuitem = true;
      }});
      selector.get_input_element().parent().bind({"mousedown touchstart": function(e) {
        return annotator.hit_menuitem = true;
      }});
      selector.get_input_element().parent().change(function(e) {
    ...
    BBoxAnnotator.prototype.add_entry = function(entry) {
      ...
      $("<div><\/div>").appendTo(close_button).html("<a style='color:#fff' href='javascript:void()'>\u00D7</a>"/*x*/).css({
        "display": "block",
        "text-align": "center",
        "width": "32px",
        "position": "absolute",
      ...
      annotator = this;
      close_button.bind({"mousedown touchstart": function(e) {
